...
public class PlayerController : MonoBehaviour {
    ...
    private float _increaseFactorForSpeed = 10f;
    ...
    void Start() {
        if (_quantityOfCursorZones < 3 || _quantityOfCenterZones < 1 || _quantityOfCursorZones - _quantityOfCenterZones < 2 ||
            ...
            (_quantityOfCursorZones % 2 != 0 && _quantityOfCenterZones % 2 == 0)) {
            Debug.LogError("Bad zones values!");
        }

        _rb = GetComponent<Rigidbody2D>();

        if (_rb == null) {
            Debug.LogError("Rigidbody 2D getting error!");
        }
    }
    ...
    private void DegreeMovement(int value, int maximumValue) {
        float singleStep = _maxSpeed / (float) Math.Pow(_degreeGround, maximumValue);

        if (value < 0) {
            singleStep = -singleStep;
        }

        SetVelocity(singleStep * (float) Math.Pow(_degreeGround, Math.Abs(value)));
    }

    private void DivisionMovement(int value, int maximumValue) {
        float singleStep = (float) _maxSpeed / maximumValue;
